...
const Navbar = () => {
  ...
  return (
    <nav className="w-full sticky top-0 z-50 shadow-md bg-white">
      <div className="container mx-auto flex items-center justify-between py-3 px-6">
        ...
        <ul className="hidden md:flex space-x-6 font-medium text-gray-700">
          {navItems.map((item, index) => (
            <li key={index} className="hover:text-pink-500 cursor-pointer transition">
        ...
        <div className="hidden md:block">
          <Link to="/contact">
            <button className="bg-pink-600 text-white px-5 py-2 rounded shadow hover:bg-pink-700 transition flex items-center gap-2">
      ...
      {/* Drawer Menu */}
      <div className={`fixed top-0 left-0 h-full w-64 bg-white shadow-lg z-50 transform transition-transform duration-500 ease-in-out ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="flex flex-col p-6 space-y-4">
          {navItems.map((item, index) => (
            <Link key={index} to={item.path} className="block text-gray-700 font-medium hover:text-pink-600" onClick={() => setMenuOpen(false)}>
              {item.name}
            </Link>
          ))}
